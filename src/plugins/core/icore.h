...
//
// Extracted from Qt Creator (coreplugin/icore.h).
// Full public interface — implemented in icore.cpp (stub now, full MainWindow
// integration in PR-1 step 4).
...
public:
    explicit ICore(Internal::MainWindow *mw = nullptr);
    ~ICore() override;

    // PR-1 step 4 will wire this up properly; for now CorePlugin can set
    // the host main window pointer so ActionManager can find it.
    static void setMainWindow(Internal::MainWindow *mw);
    // Convenience overload for hosts that own a real QMainWindow*.
    static void setMainWindow(QMainWindow *mw);
    // QWidget-based host (e.g. QWK + custom titlebar window). mainWindow()
    // will return nullptr for these hosts; ActionManager tolerates null.
    static void setMainWindow(QWidget *w);
    // Host can register its menu/status bar so plugins can attach without
    // requiring a QMainWindow parent.
    static void setMenuBar(QMenuBar *mb);
    static QMenuBar *menuBar();
    static void setStatusBar(QStatusBar *sb);
    // Host can register a container widget where mode pages / dock manager
    // are embedded.
    static void setModeBarContainer(QWidget *w);
    static QWidget *modeBarContainer();
    static void setCentralContainer(QWidget *w);
    static QWidget *centralContainer();
